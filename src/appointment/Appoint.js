...
//     const fetchAppointments = async () => {
//       try {
//         const response = await axios.get(`http://127.0.0.1:8000/clinic/available-times/?doctor=${id}`);
//         setAppointments(response.data);
//         setSelectedAvailableTimeId(response.data.id);
//         console.log("2",setSelectedAvailableTimeId);
//         setDoctorId(id);
//         console.log("1",setDoctorId);
//       } catch (error) {
//         console.error("Error fetching appointments:", error);
//         setError("Failed to fetch appointments. Please try again.");
//       } finally {
...
//   //     id: Date.now(),
//   //     date: selectedDay.date,
//   //     time_range: selectedTime, // يحتوي على start_time و end_time
//   //     patientName,
//   //     patientPhone,
//   //     bookedAt: new Date().toLocaleString(),
//   //   };
...
//       const newAppointment = {
//         patient: patientId,
//         // bookedAt: new Date().toISOString(), // Use ISO format for consistency
//         date: selectedDay.date,
//         time_range: selectedTime,
//         doctor: doctorId,
...
// function Appoint() {
//   const [bookedAppointments, setBookedAppointments] = useState([]); // حالة محلية للمواعيد المحجوزة
//   const [selectedDay, setSelectedDay] = useState(null);
//   const [selectedTime, setSelectedTime] = useState(null);
//   const [patientName, setPatientName] = useState("");
//   const [patientPhone, setPatientPhone] = useState("");
//   const [appointments, setAppointments] = useState([]); // الأوقات المتاحة
//   const [showConfirm, setShowConfirm] = useState(false);
//   const [showToast, setShowToast] = useState(false);
//   const [formErrors, setFormErrors] = useState({});
//   const [loading, setLoading] = useState(true);
...
//   // جلب الأوقات المتاحة والمواعيد المحجوزة عند تحميل الكومبوننت
//   useEffect(() => {
//     const fetchData = async () => {
//       try {
//         // جلب الأوقات المتاحة
//         const availableResponse = await axios.get(`http://127.0.0.1:8000/clinic/available-times/`);
//         setAppointments(availableResponse.data);

//         // جلب المواعيد المحجوزة
//         const bookedResponse = await axios.get(`http://127.0.0.1:8000/clinic/booked-appointments/`);
...
//     try {
//       // إرسال الموعد الجديد إلى API
//       const response = await axios.post(`http://127.0.0.1:8000/clinic/book-appointment/`, newAppointment);
//       // تحديث المواعيد المحجوزة محليًا باستخدام الاستجابة من API (مع افتراض أن API يعيد الموعد مع id)
...
//   // دالة لجلب جميع الأوقات المتاحة ليوم معين
//   const getTimeSlotsForDay = (dayDate) => {
//     return appointments
//       .filter((appt) => appt.date === dayDate)
//       .map((appt) => ({
